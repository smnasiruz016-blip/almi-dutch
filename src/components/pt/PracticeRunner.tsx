...
// submits each to /api/pt/submit for deterministic grading, shows per-item
// correctness, and closes with an honest per-skill readiness readout. All labels
// are framed as a "practice estimate" — never an official CAPLE result.

import { useState } from "react";
import type { PortugueseSkill, PortugueseVariant, CapleSkill } from "@/lib/pt/types";
import { skillReadout, readinessFromPct } from "@/lib/pt/caple";
...
export function PracticeRunner({
  ...
  if (done) {
    ...
    const readout = skillReadout(skill as CapleSkill, points, maxPoints);
    ...
    return (
      <div className="space-y-5 rounded-2xl border border-almi-bg-peach bg-almi-paper p-6">
        ...
        <p className="text-xs text-almi-text-muted">
          This is a practice estimate against the CAPLE Suficiente threshold, not an official
          classification.
        </p>
        <button
          type="button"
