...
// per-skill readouts. Always labelled "estimate, not the official CvTE/DUO
// result". Objective skills are auto-graded; productive skills (Writing/Speaking/
// ONA) are self-rated for now (AI grading arrives in a later phase).
...
export function MockRunner({
  ...
  async function finish() {
    setGrading(true);
    const perSkill: Partial<Record<DutchSkill, { points: number; max: number }>> = {};
    const prodPct: Partial<Record<DutchSkill, number[]>> = {};
  ...
  return (
    <div className="space-y-3">
      <textarea
        ...
        rows={6}
        placeholder="Schrijf of maak hier je aantekeningen…"
...
function MockResult({
  ...
  agg: { meanPct: number; overall: string; label: string; weakest: DutchSkill | null; allClear: boolean };
}) {
  return (
    <div className="space-y-5 rounded-2xl border border-almi-bg-peach bg-almi-paper p-6">
      ...
      <p className="text-xs text-almi-text-muted">
        This is a practice estimate, not the official CvTE/DUO result. For NT2, the Diploma is awarded
        only when all four parts are passed against the official criteria.
      </p>
    </div>
  );
}
