...
// Productive composer for the gated skills (Writing / Speaking) and the
// Inburgering ONA reflection. Shows the stimulus + criteria, a writing textarea
// with a live length counter (or a speaking notes area with timing guidance).
// Writing / Speaking answers get an honest AI readiness estimate (CLEAR/
// BORDERLINE/BELOW) with feedback against the exam's own criteria — NEVER an
// official CvTE/DUO score. When AI grading isn't available (key not provisioned
// yet, or a transient hiccup) it falls back to a clearly-labelled self-rating.
...
import { SKILL_LABELS } from "@/lib/nl/registry";
import {
  submitAttempt,
  gradeProductive,
  type ProductiveItem,
...
function isWritingType(t: string): boolean {
  return t === "WRITING_PROMPT" || t === "ONA_TASK";
}
...
function readDisplay(item: ProductiveItem): {
  ...
  switch (item.taskType) {
    ...
    case "ONA_TASK": {
      const p = item.payload as OnaTaskPayload;
      return { ...base, stimulus: p.scenario ?? null, criteria: p.criteria ?? [], prompts: p.prompts ?? [] };
    }
    default:
      return base;
  }
      ...
      <div className="space-y-4 rounded-2xl border border-almi-bg-peach bg-almi-bg-peach/30 p-5">
        ...
        <div>
          ...
          <textarea
            ...
            rows={writing ? 8 : 5}
            disabled={saved}
            placeholder={writing ? "Schrijf hier je antwoord…" : "Jot down what you would say…"}
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-almi-bg-peach bg-almi-paper p-5">
      ...
      <p className="text-xs text-almi-text-muted">
        An honest practice estimate against the exam&apos;s criteria — not the official CvTE/DUO result.
      </p>
    </div>
  );
}
...
function Summary({
  ...
  return (
    <div className="space-y-5 rounded-2xl border border-almi-bg-peach bg-almi-paper p-6">
      ...
      <p className="text-xs text-almi-text-muted">
        This is a self-assessment for practice only — an estimate, not the official CvTE/DUO result.
      </p>
