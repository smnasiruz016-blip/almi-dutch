...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  return {
    title: { absolute: `Study ${s.name} in the Netherlands — Dutch-language pathway | AlmiDutch` },
    description: `The Dutch-language route for studying ${s.name} in the Netherlands — typical NT2 level and honest readiness practice, by country of origin.`,
    alternates: { canonical: `/study-in-netherlands/${s.slug}` },
  };
}
    ...
    <main className="bg-almi-bg text-almi-text">
      <div className="mx-auto max-w-4xl px-6 py-12">
        <nav aria-label="Breadcrumb" className="mb-6 text-xs text-almi-text-muted">
          <Link href="/study-in-netherlands" className="hover:text-almi-coral">Study in the Netherlands</Link> / {s.name}
        </nav>
        <h1 className="text-3xl font-semibold text-almi-ink sm:text-4xl">Study {s.name} in the Netherlands</h1>
        <p className="mt-3 max-w-2xl text-base text-almi-text">
          Dutch universities and hogescholen offer {s.name} programmes; most Dutch-taught courses ask for
          around B1–B2. Pick your country of origin for the language pathway, or start practising the NT2 exams now.
        </p>
        <div className="mt-6">
          <Link href="/exams" className="text-sm font-semibold text-almi-coral hover:underline">See the Dutch exam guides →</Link>
        </div>
