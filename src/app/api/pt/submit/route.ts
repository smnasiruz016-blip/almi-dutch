...
// CAPLE engine and echoes productive self-ratings. DB persistence of attempts is
...
import { gradeObjective } from "@/lib/pt/caple";
import { isObjectiveTask, isFreeSkill } from "@/lib/pt/types";
import type {
  ObjectiveAnswer,
  PortugueseSkill,
  PortugueseTaskType,
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  if (itemId) {
    try {
      await prisma.portugueseAttempt.create({
        data: {
          userId: user.id,
          itemId,
