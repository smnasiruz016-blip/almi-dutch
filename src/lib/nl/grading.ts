// AlmiDutch scoring engine — per-skill READINESS estimate. NT2 parts and the
// Inburgering exams are pass/fail against official criteria; we do NOT fabricate
// an official CvTE/DUO result. We score each skill's objective items
 ...
 * weakest skill — never claim an official classification. For NT2 we also surface
 * whether every graded part reads CLEAR (the "all four parts" pass shape).
 */
export function aggregateReadout(readouts: SkillReadout[]): {
  meanPct: number;
  overall: Readiness;
  label: string;
  weakest: DutchSkill | null;
  allClear: boolean;
} {
  ...
  const meanPct = graded.length
    ...
    : 0;
  let weakest: DutchSkill | null = null;
  let low = Infinity;
