...
//
//  1. SELF-ORIGIN. src/data/seo/countries.json listed Netherlands itself, so the
//     matrix advertised "Study in Netherlands from Netherlands". almi-dutch and
//     almi-icelandic had the identical bug. axes.ts even carried the comment
//     "origins only (Netherlands excluded)" — the label was localized, the fact was
...
const SELF_ISO = "NL"; // this product's own country — never a valid origin of itself
